...
export default function BookingPageClient({ doctors, contactInfo, isGuest }: BookingPageClientProps) {
  const [mode, setMode] = useState<'selection' | 'ai' | 'manual'>('selection');
  const [initialData, setInitialData] = useState<BookingInitialData | undefined>(undefined);

  const handleAIComplete = (result: ConsultationResult) => {
    // Map result to BookingInitialData
    let doctorId: string | undefined;
    if (result.preferredDoctor) {
        const doctor = doctors.find(d => d.name.toLowerCase().includes(result.preferredDoctor!.toLowerCase()));
        if (doctor) {
            doctorId = doctor.id;
        }
    }
    ...
    setInitialData({
        notes: `Symptoms: ${result.summary}\n\nPreferences: ${result.preferredDate || 'None'} at ${result.preferredTime || 'None'}`,
        doctorId: doctorId,
    });
    setMode('manual');
  };
  ...
  if (mode === 'selection') {
    return (
      <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto mt-10">
        <Card className="hover:shadow-lg transition-shadow cursor-pointer border-blue-200 bg-blue-50/50" onClick={() => setMode('ai')}>
          <CardHeader>
            ...
            <CardDescription>
              Talk to our AI assistant. It will ask about your symptoms and help you fill out the form.
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="hover:shadow-lg transition-shadow cursor-pointer" onClick={() => setMode('manual')}>
          <CardHeader>
            ...
            <CardDescription>
              Fill out the appointment form yourself. Choose your doctor and time slot directly.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button variant="outline" className="w-full">Fill Form Manually</Button>
          </CardContent>
        </Card>
      </div>
    );
